...
function workspaceMetadata(root: string) {
  const backendFile = path.join(root, 'webmux/backend/package.json');
  const frontendFile = path.join(root, 'webmux/frontend/package.json');
  const lockFile = path.join(root, 'webmux/package-lock.json');
  const backend = read(backendFile), frontend = read(frontendFile), lock = read(lockFile);
  const packages = object(lock.packages, `${lockFile}: packages`);
  const lockedBackend = object(packages.backend, `${lockFile}: backend`);
  const lockedFrontend = object(packages.frontend, `${lockFile}: frontend`);
  return { backendFile, frontendFile, lockFile, backend, frontend, lock, lockedBackend, lockedFrontend };
}
...
switch (action) {
  ...
  case 'validate': {
    ...
    const expected = version(value.backend, 'backend/package.json');
    for (const [label, doc] of [
      ['frontend/package.json', value.frontend], ['lock backend', value.lockedBackend], ['lock frontend', value.lockedFrontend],
    ] as const) assert.equal(version(doc, label), expected, `Version mismatch: ${label} != backend/package.json`);
  ...
  case 'bump': {
    ...
    // change; the root workspace and dependency versions remain untouched.
    for (const doc of [value.backend, value.frontend, value.lockedBackend, value.lockedFrontend]) {
      version(doc, 'workspace'); doc.version = argument;
    }
    for (const [file, doc] of [[value.backendFile, value.backend], [value.frontendFile, value.frontend], [value.lockFile, value.lock]] as const) {
